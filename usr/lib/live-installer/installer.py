...
class InstallerEngine:
    ...
    def init_install(self, setup):        
        ...
        chroot_exec("useradd -s %s -c \'%s\' -G sudo,adm,dialout,audio,video,cdrom,floppy,dip,plugdev,lpadmin,sambashare -m %s" % ("/bin/bash", setup.real_name, setup.username))
        os.system("chroot /target/ /bin/bash -c \"shopt -s dotglob && cp -R /etc/skel/* /home/%s/\"" % setup.username)
        chroot_exec("chown -R %s:%s /home/%s" % (setup.username, setup.username, setup.username))
